...
var tileid = m.stream(0);
var monitorwidth = 600;
...
var CTRL = {
	zoomIn: function zoomIn() {
		var futurezoom = MODEL.zoom() + zoomstep;
	...
	openModal: function openModal(ch) {
		MODEL.modalopen(true);
		MODEL.modalcomponent(ch);
	},
	fillData: function fillData() {
		MODEL.session.data(new Array(MODEL.dimensions.height()).fill(new Array(MODEL.dimensions.width()).fill(MODEL.session.tiles[0])));
	},
	refillData: function refillData() {
		var data = MODEL.session.data();
		for (var y = 0; y < MODEL.dimensions.height(); y++) {
			if (data[y] == null) {
				data.push(new Array(MODEL.dimensions.width()).fill(MODEL.session.tiles[0]));
			}
		}
		for (var _y = 0; _y < MODEL.dimensions.height(); _y++) {
			for (var x = 0; x < MODEL.dimensions.width(); x++) {
				if (data[_y][x] == null) {
					data[_y][x] = MODEL.session.tiles[0];
				}
			}
		}
	},
	redrawCanvas: function redrawCanvas() {
		//very fake, don't blame me, i suck. Change of width in stream will re-plant the canvas element and redraw
...
module.exports = {
	CTRL: CTRL, MODEL: MODEL
};

},{"mithril":2,"mithril-stream":1}],6:[function(require,module,exports){
"use strict";
...
},{"./frame.js":5,"./modules/App.js":7,"mithril":2,"mithril-stream":1}],7:[function(require,module,exports){
"use strict";
...
var App = {
	...
	view: function view(vnode) {
		return m(".screen", [m(Monitor), m(Tools), MODEL.modalopen() ? m(Modal) : ""]);
	}
};

module.exports = App;

},{"../frame.js":5,"./Modal.js":12,"./Monitor.js":13,"./Tools.js":17,"mithril":2,"mithril-stream":1}],8:[function(require,module,exports){
"use strict";
...
module.exports = function (dom) {
	...
	//REUSABLE FUNCTIONS
	var createCanvas = function createCanvas(w, h) {
	...
	var resetPreview = function resetPreview() {
		var arr = new Array(MODEL.dimensions.height()).fill();
	...
	//CONSTANTS
	var $ = dom.getContext("2d");
	var w = dom.width;
	var h = dom.height;
	var image = createCanvas(w, h);
	var preview = createCanvas(w, h);
	...
	function drawTiles(data, canvas) {
		var defaultscale = MODEL.defaultscale,
		    zoom = MODEL.zoom,
		    dimensions = MODEL.dimensions;

		var tilesize = defaultscale() * zoom();
		for (var y = 0; y < data.length; y++) {
			for (var x = 0; x < data[y].length; x++) {
				var tile = data[y][x];
				var _$ = canvas.getContext('2d');
				_$.fillStyle = tile.color();
	...
	function modifyPreview(tool, tile, startpos, brushpos) {
		...
		var applyBrush = function applyBrush() {
			previewData[brushpos.y][brushpos.x] = tile;
		};
		var applySquare = function applySquare() {
			resetPreview();
			clearCanvas(preview);
			for (var y = startpos.y; y <= brushpos.y; y++) {
				for (var x = startpos.x; x <= brushpos.x; x++) {
					if (y == startpos.y || y == brushpos.y) previewData[y][x] = tile;
					if (x == startpos.x || x == brushpos.x) previewData[y][x] = tile;
				}
			}
		};
		switch (tool) {
	...
	function drawPreview(data, canvas) {
		...
		for (var y = 0; y < data.length; y++) {
			for (var x = 0; x < data[y].length; x++) {
				var tile = data[y][x];
				var _$2 = canvas.getContext('2d');
				var color = 'rgba(0,0,0,0)';
				tile == undefined ? void 0 : color = tile.color();
				_$2.fillStyle = color;
				_$2.fillRect(x * tilesize, y * tilesize, tilesize, tilesize);
			}
		}
		$.drawImage(image, 0, 0);
	...
	function applyPreview() {
		var data = MODEL.session.data;
		var arr = data().map(function (row, y, arr) {
			return row.map(function (col, x, row) {
				var pdvalue = previewData[y][x];
				var rvalue = pdvalue != "undefined" && pdvalue != null ? pdvalue : col;
				return rvalue;
			});
		});
		data(arr);
...
},{"../frame.js":5,"mithril":2,"mithril-stream":1}],9:[function(require,module,exports){
"use strict";

var m = require("mithril");
m.stream = require("mithril-stream");
var frame = require("../frame.js");
var CanvasControl = require("./Canvas.js");
...
var CanvasMonitor = {
	oninit: function oninit(vnode) {
		this.ctrl = {
			...
			zoomInput: function zoomInput(value) {
				if (value >= 1) MODEL.zoom(value);else return;
			},
	...
	view: function view(vnode) {
		return m("div.canvas-wrapper", {
		...
		}), m("div.control-bar", [m("div.zoom", [m("input", {
			type: "number",
			...
			title: "Redraw"
		}, m("span.fas.fa-sync-alt")), m("button", {
...
},{"../frame.js":5,"./Canvas.js":8,"./Settings.js":14,"mithril":2,"mithril-stream":1}],10:[function(require,module,exports){
"use strict";
...
var CreateModal = {
	oninit: function oninit(vnode) {
		...
		this.acceptInput = function () {
			var _dataStreams = this.dataStreams,
			    title = _dataStreams.title,
			    symbol = _dataStreams.symbol,
			    color = _dataStreams.color;

			var acceptTile = title().length > 0 && symbol().length > 0;
			if (acceptTile) {
		...
		this.pushTile = function (title, symbol, color) {
			var id = MODEL.tileid();
	...
	view: function view(vnode) {
		return m("div.create-tile", [m("div.create-inputs", [m("div.color-input", [m("label[for='new-tile-color']", "Color"), m("input[type='color']", {
			id: "new-tile-color",
			onchange: m.withAttr("value", this.dataStreams.color),
			value: this.dataStreams.color()
		})]), m("div.text-inputs", [m("div.input", [m("label[for='new-tile-name']", "Name"), m("input[type='text']", {
			id: "new-tile-name",
			oninput: m.withAttr("value", this.dataStreams.title),
			value: this.dataStreams.title()
		})]), m("div.input", [m("label[for='new-tile-symbol']", "Symbol"), m("input[type='text']", {
			id: "new-tile-symbol",
			oninput: m.withAttr("value", this.dataStreams.symbol),
			value: this.dataStreams.symbol(),
			maxlength: 1
		})])])]), m("div.buttons", [m("button", {
			onclick: function () {
...
},{"../frame.js":5,"mithril":2,"mithril-stream":1}],11:[function(require,module,exports){
"use strict";
...
var EditModal = {
	oninit: function oninit(vnode) {
		...
		this.acceptInput = function () {
			if (_this.fake.title().length > 0 && _this.fake.symbol().length > 0) _this.submitEdit();else {
	...
	view: function view(vnode) {
		return m("div.edit-tile", [m("h2", "Edit Tile"), m("div.create-inputs", [m("div.color-input", [m("label[for='new-tile-color']", "Color"), m("input[type='color']", {
			id: "new-tile-color",
...
},{"../frame.js":5,"mithril":2,"mithril-stream":1}],12:[function(require,module,exports){
"use strict";
...
var Modal = {
	...
	view: function view(vnode) {
		return m("div.modal-wrapper", [m("div.modal-backdrop", {
...
},{"../frame.js":5,"mithril":2,"mithril-stream":1}],13:[function(require,module,exports){
"use strict";
...
var Modal = require("./Modal.js");
var CreateModal = require("./CreateModal.js");
var CanvasMonitor = require("./CanvasMonitor.js");
...
var Monitor = {
	oninit: function oninit(vnode) {
		this.ctrl = {
			createTile: function createTile(e) {
				CTRL.openModal(CreateModal);
			},
			saveSession: function saveSession(e) {
				console.log(e);
				return;
			},
			getOutput: function getOutput(e) {
				var output = MODEL.session.data().map(function (row) {
				...
				var component = {
					view: function view(vnode) {
						return m("div.output-modal", [m("textarea", {
							style: "\n\t\t\t\t\t\t\t\t\twidth: 320px;\n\t\t\t\t\t\t\t\t\theight: 320px;\n\t\t\t\t\t\t\t\t",
							oncreate: function oncreate(vnode) {
								vnode.dom.select();
							}
	...
	view: function view(vnode) {
		var that = this;
		return m("div.monitor", {}, [m("h1", "ASCII Level Maker"), m(CanvasMonitor), m(Tiles), m("div.buttons", [buttons.map(function (b) {
			return m("button", {
				class: b.class,
...
},{"../frame.js":5,"./CanvasMonitor.js":9,"./CreateModal.js":10,"./Modal.js":12,"./Tiles.js":16,"mithril":2,"mithril-stream":1}],14:[function(require,module,exports){
"use strict";
...
var Settings = {
	oninit: function oninit(vnode) {
		...
		this.nodes = {
			Field: {
				oninit: function oninit(vnode) {
					this.ctrl = {
						processInput: function processInput(value) {
							var str = value.replace(/\D+/g, ''); //only numbers
				...
				view: function view(vnode) {
					var lowname = vnode.attrs.name.toLowerCase().replace(" ", "-");
					return m("div.input-wrapper", [m("label", {
						for: "input-" + lowname
					}, vnode.attrs.name), m("input", {
		...
		this.ctrl = {
			applySettings: function applySettings(event) {
				this.displayWarning = false;
	...
	view: function view(vnode) {
		var that = this;
		return m("div.settings-modal", {}, [m("h2", "Settings"), m("div.text-inputs", [this.fields.map(function (fieldData) {
			return m(that.nodes.Field, fieldData);
		})]), m("div.buttons", [m("button", {
			onclick: this.ctrl.applySettings.bind(this)
		}, "Apply")]), this.displayWarning ? m(this.nodes.Warning) : ""]);
	}
};

module.exports = Settings;

},{"../frame.js":5,"mithril":2,"mithril-stream":1}],15:[function(require,module,exports){
"use strict";
...
var frame = require("../frame.js");
var MODEL = frame.MODEL;
var EditModal = require("./EditModal.js");
...
var Tile = {
	oninit: function oninit(vnode) {
		...
		this.selectTile = function (e) {
			...
			MODEL.session.tiles.forEach(function (tile) {
		...
		this.editTile = function (e) {
			var that = this;
			var openModal = function openModal(ch) {
				MODEL.modalopen(true);
				MODEL.modalcomponent(ch);
			};
			openModal({
	...
	view: function view(vnode) {
		...
		}), m("div.title", this.original.title()), m("div.info", [m("span.symbol", this.original.symbol()), m("span.edit-button", {
...
},{"../frame.js":5,"./EditModal.js":11,"mithril":2,"mithril-stream":1}],16:[function(require,module,exports){
"use strict";
...
var Tiles = {
	view: function view(vnode) {
		var that = this;
		return m("div.tiles", [m("div.wrapper", [m("div.tiles-list", [MODEL.session.tiles.map(function (el, ind, arr) {
			return m(Tile, {
				id: el.id()
			});
		})])])]);
...
},{"../frame.js":5,"./EditModal.js":11,"./Tile.js":15,"mithril":2,"mithril-stream":1}],17:[function(require,module,exports){
"use strict";
...
var imageLocation = "website-assets/images/<image>.png";
var Tools = {
	oninit: function oninit(vnode) {
		var that = this;
		this.nodes = {
			Tool: {
				view: function view(vnode) {
					var className = "tool tool-" + vnode.attrs.name.toLowerCase() + " " + (vnode.attrs.selected ? "selected" : "");
					var name = vnode.attrs.name;
					return m("div", {
						className: className,
						"data-tool": name,
						onclick: that.ctrl.selectTool
					}, [m("img", { src: imageLocation.replace("<image>", name.toLowerCase()) }), m("span", name)]);
				}
			}
		};
		this.ctrl = {
			selectTool: function selectTool(e) {
				var el = e.currentTarget;
				var tool = el.dataset.tool;
				MODEL.currenttool(tool);
				return;
			},
			toggleAlwaysOpen: function toggleAlwaysOpen(value) {
	...
	view: function view(vnode) {
		...
		}, [m("div.wrapper", [m("span", "Tools"), MODEL.tools.map(function (name) {
			return m(that.nodes.Tool, {
				name: name,
				selected: name == MODEL.currenttool()
			});
		}), m("div.always-open", [m("input", {
			type: "checkbox",
...
},{"../frame.js":5,"mithril":2,"mithril-stream":1}]},{},[6]);
